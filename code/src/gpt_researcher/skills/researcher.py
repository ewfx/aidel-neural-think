...
class ResearchConductor:
    ...
    def __init__(self, researcher):
        ...
        self.keywords= [
        "fraud", "scam", "lawsuit", "investigation", "whistleblower", "regulatory action",
        "financial misconduct", "SEC filing", "legal case", "class action", "accounting fraud",
        "Ponzi scheme", "insider trading", "whistleblower complaint", "false advertising",
        "consumer fraud", "regulatory fine", "antitrust violation", "money laundering",
        "embezzlement", "tax evasion", "corporate misconduct"
    ]
    ...
    async def conduct_research(self):
        ...
        if self.researcher.source_urls:
            ...
            if self.researcher.complement_source_urls:
                ...
                additional_research = await self._get_context_by_web_search(self.researcher.query, [], self.researcher.query_domains,company_name=self.researcher.tone) #tone stores company name
                research_data += ' '.join(additional_research)

        elif self.researcher.report_source == ReportSource.Web.value:
            self.generate_reportlogger.info("Using web search")
            research_data = await self._get_context_by_web_search(self.researcher.query, [], self.researcher.query_domains,company_name=self.researcher.tone) #tone stores company name
        ...
        # ... rest of the conditions ...
            ...
            research_data = await self._get_context_by_web_search(self.researcher.query, document_data, self.researcher.query_domains,enable_subqueries=False)
    ...
    async def _get_context_by_web_search(self, query,scraped_data: list | None = None, query_domains: list | None = None,enable_subqueries:bool =True,company_name=None):
        """
        ...
        # Generate Sub-Queries including original query
            ...
            if len(sub_queries)>1 and company_name:
                sub_queries=sub_queries[1]
                sub_queries.append(f"{company_name} wikipedia")

                sub_queries_gen = [f"{company_name} {keyword}" for keyword in self.keywords]
                sub_queries.extend(sub_queries_gen)

                


            with open('/home/anonymousa/Projects/wf-hackathon/gpt-researcher/backend/report_type/output/sub_queries.txt', 'a') as file:
